    ...
    companion object {
        fun fromClaims(claims: FirebaseUserClaims): FirebaseHitobitoUserRole {
            ...
            return entries.find { it.role == roleClaim }
                ?: throw PfadiSeesturmError.AuthError("Du hast keine Berechtigung, um dich in der Pfadi Seesturm App anzumelden. Melde dich erneut via MiData an.")
        }
        fun fromRole(role: String): FirebaseHitobitoUserRole {
            return entries.find { it.role == role }
                ?: throw PfadiSeesturmError.AuthError("Unbekannte Rolle. Melde dich erneut via MiData an.")
        }
    }
}